...
FALLBACK_MSGS = {
    "en": {
        "unknown": "I don’t have specific instructions in my offline knowledge base. Please connect to the internet to get more help from the AI assistant.",
        "done": "Those are the steps. If the problem continues or is severe, please seek emergency care."
    },
    "ti": {
        "unknown": "ኣብቲ ካብ መስመር ወጻኢ ዝኾነ ፍልጠተይ ፍሉይ መምርሒ የብለይን። በጃኹም ካብ ሓጋዚ AI ዝያዳ ሓገዝ ንምርካብ ምስ ኢንተርነት ተራኸቡ።",
        "done": "እቶም ስጉምትታት እዮም። እቲ ጸገም እንተቐጺሉ ወይ ከቢድ እንተኾይኑ በጃኹም ህጹጽ ክንክን ድለዩ።"
    }
}
...
def load_kb(lang="en"):
    ...
    if not os.path.exists(path):
        path = os.path.join(KB_DIR, "en.json")  # fallback to English
...
def start_intent(session, intent, kb):
    ...
    if followups:
        ...
        return {
            "text": step_text,
            "followup_question": session["followup_meta"]["question"],
            "awaiting": True,
            "has_next": len(session["steps"]) > 1
        }
    else:
        step_text = session["steps"][0] if session["steps"] else ""
        return {
...
def advance_step(session, kb):
    ...
    if session["current_step"] + 1 < len(session["steps"]):
        ...
        return {
            ...
            "awaiting": False,
            "done": False,
            "has_next": session["current_step"] + 1 < len(session["steps"])
        }
    else:
    ...
    if action == "next":
        ...
        if session.get("awaiting_followup"):
            ...
            if intent:
                ...
                resp = {"text": FALLBACK_MSGS[lang]["unknown"], "awaiting": False}
